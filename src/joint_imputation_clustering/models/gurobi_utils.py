...
from joint_imputation_clustering.utils.seeding import normalize_gurobi_seed
...
model.Params.Seed = normalize_gurobi_seed(seed)
...
absolute_gap = (
abs(objective - bound)
if has_solution and np.isfinite(bound)
else np.nan
)
...
raise RuntimeError(
f"{stage} found no feasible solution; "
f"status={status_name(model.Status)}"
)
